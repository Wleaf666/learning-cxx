...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        {
            vec.shrink_to_fit();
            auto capacity_after_shrink = vec.capacity();
            ASSERT(capacity_after_shrink >= vec.size(), "Fill in the correct value.");
            vec.clear();
            ASSERT(vec.empty(), "`vec` is empty now.");
            ASSERT(vec.size() == 0, "Fill in the correct value.");
            ASSERT(vec.capacity() == capacity_after_shrink, "Fill in the correct value.");
        }
    }
    {
